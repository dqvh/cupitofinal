  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-evergreen px-5 text-paper">
      ...
      <div key={shakeKey} className={`relative w-full max-w-sm rounded-[24px] border-2 border-paper/15 bg-pine/90 p-8 shadow-2xl backdrop-blur-md ${error ? "shake" : ""}`}>
        ...
        {isLocked && (
          <div className="mt-4 flex items-center justify-center gap-1.5 rounded-xl border border-rose-400/30 bg-rose-500/10 p-3 text-center text-xs font-semibold text-rose-300">
            <Shield className="h-4 w-4 shrink-0" /> Acceso suspendido temporalmente por seguridad. Podrás intentar de nuevo en {minutesLeft} minuto{minutesLeft === 1 ? "" : "s"}.
          </div>
        )}
...
function Console() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] text-[#1D1D1F]">
      {/* Header Superior Blanco Minimalista */}
      <header className="sticky top-0 z-40 border-b border-black/[0.08] bg-white text-[#1D1D1F] shadow-xs">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3.5 sm:px-8">
          <div className="flex items-center gap-3">
            <img src="/cupito-logo.png" width="32" height="32" alt="" className="rounded-lg shrink-0" />
            <div>
              <div className="flex items-center gap-2">
                <p className="font-display text-lg font-bold tracking-tight text-[#1D1D1F] leading-tight">Central Cupito</p>
                <span className="rounded-full bg-[#F5F5F7] px-2 py-0.5 text-[10px] font-bold text-[#1D1D1F]">Super-Admin</span>
              </div>
              <p className="text-[11px] text-[#6E6E73]">Plataforma y administración general</p>
            </div>
          </div>

          {/* Segmented Control Tabs */}
          <nav aria-label="Secciones de la Central" className="hidden sm:flex items-center gap-1 rounded-full bg-[#F5F5F7] p-1 border border-black/[0.06]">
            {([
              { id: "resumen", label: "Resumen" },
              { id: "negocios", label: `Negocios (${stats.locales})` },
              { id: "dinero", label: "Dinero" },
              { id: "alertas", label: `Alertas${stats.expired + stats.expiring7 + stats.grace > 0 ? ` (${stats.expired + stats.expiring7 + stats.grace})` : ""}` },
              ...
              <button
                ...
                className={`rounded-full px-4 py-1.5 text-xs font-semibold transition-all ${
                  tab === t.id ? "bg-black text-white font-bold shadow-xs" : "text-[#6E6E73] hover:text-[#1D1D1F]"
                }`}
              >
                {t.label}
              </button>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-black hover:bg-neutral-800 px-4 py-2 font-display text-xs font-bold text-white shadow-xs transition-all active:scale-95"
            >
              <Plus size={14} /> <span>Agregar negocio</span>
            </button>
            <button
              onClick={() => { adminLogout(); window.location.hash = "#/"; }}
              className="inline-flex items-center gap-1 rounded-full border border-black/10 px-3 py-2 font-display text-xs font-semibold text-[#6E6E73] hover:bg-[#F5F5F7] hover:text-[#1D1D1F] transition-colors"
              title="Cerrar sesión"
            >
              <LogOut size={14} />
            </button>
        ...
        {/* Mobile Tab Bar */}
        <nav aria-label="Secciones de la Central (móvil)" className="flex sm:hidden overflow-x-auto border-t border-black/[0.06] px-4 py-2 gap-1.5 no-scrollbar">
          {([
            { id: "resumen", label: "Resumen" },
            ...
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-semibold shrink-0 transition-all ${
                tab === t.id ? "bg-black text-white font-bold" : "bg-[#F5F5F7] text-[#6E6E73]"
              }`}
            >
              {t.label}
            </button>
      ...
      <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
        {/* Banner de resumen y estado */}
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-black/[0.08] bg-white p-5 shadow-xs">
          <div>
            <h1 className="font-display text-xl font-extrabold text-[#1D1D1F]">
              Estado general: {stats.locales} negocios activos · {fmtMoney(stats.estimatedMonthly)}/mes
            </h1>
            <p className="text-xs text-[#6E6E73] mt-0.5">
              Control de suscripciones, métricas y soporte centralizado.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => setShowSyncModal(true)}
              className="inline-flex items-center gap-1.5 rounded-full border border-black/10 bg-[#F5F5F7] px-3.5 py-1.5 text-xs font-semibold text-[#1D1D1F] hover:bg-neutral-200/70 transition-colors"
            >
              <span className={`h-2 w-2 rounded-full ${isCloudSyncActive ? "bg-[#16A34A]" : "bg-amber-500 animate-pulse"}`} />
              <span>{isCloudSyncActive ? "Nube activa" : "Modo local"}</span>
            </button>
            <button
              onClick={() => exportAdminCSV(users, getData)}
              className="inline-flex items-center gap-1.5 rounded-full border border-black/10 bg-white px-3.5 py-1.5 text-xs font-semibold text-[#1D1D1F] hover:bg-[#F5F5F7] transition-colors"
            >
        ...
        {tab === "resumen" && (
          <div className="pop-in space-y-6">
            {/* 4 Tarjetas de Métricas Esenciales (KPIs) */}
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-2xl border border-black/[0.08] bg-white p-5 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-[#6E6E73]">MRR Mensual</span>
                  <CreditCard size={16} className="text-[#16A34A]" />
                </div>
                <p className="mt-2 font-display text-2xl sm:text-3xl font-extrabold text-[#1D1D1F]">{fmtMoney(stats.estimatedMonthly)}</p>
                <p className="mt-1 text-xs text-[#6E6E73]">{stats.paid} locales de pago · Ticket {fmtMoney(stats.avgTicket)}</p>
              </div>

              <div className="rounded-2xl border border-black/[0.08] bg-white p-5 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-[#6E6E73]">Conversión de pago</span>
                  <TrendingUp size={16} className="text-[#1D1D1F]" />
                </div>
                <p className="mt-2 font-display text-2xl sm:text-3xl font-extrabold text-[#1D1D1F]">{stats.conversion}%</p>
                <p className="mt-1 text-xs text-[#6E6E73]">De cada 100 negocios registrados pagan {stats.conversion}</p>
              </div>

              <div className="rounded-2xl border border-black/[0.08] bg-white p-5 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-[#6E6E73]">Vencen en 7 días</span>
                  <AlertCircle size={16} className={stats.expiring7 > 0 ? "text-amber-600" : "text-[#6E6E73]"} />
                </div>
                <p className="mt-2 font-display text-2xl sm:text-3xl font-extrabold text-[#1D1D1F]">{stats.expiring7}</p>
                <p className="mt-1 text-xs text-[#6E6E73]">{stats.expiring7 > 0 ? "Requieren aviso o renovación" : "Sin vencimientos próximos"}</p>
              </div>

              <div className="rounded-2xl border border-black/[0.08] bg-white p-5 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-[#6E6E73]">Turnos últimos 7 días</span>
                  <Calendar size={16} className="text-[#16A34A]" />
                </div>
                <p className="mt-2 font-display text-2xl sm:text-3xl font-extrabold text-[#1D1D1F]">{stats.last7Bookings}</p>
                <p className="mt-1 text-xs text-[#6E6E73]">{stats.todayCount} turnos hoy · {stats.bookings} totales</p>
              </div>
            </div>

            {/* Único Gráfico de Barras Nítido de Actividad (Últimos 14 días) */}
            <div className="rounded-2xl border border-black/[0.08] bg-white p-6 shadow-xs">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <h3 className="font-display text-base font-bold text-[#1D1D1F]">Actividad de reservas (últimos 14 días)</h3>
                  <p className="text-xs text-[#6E6E73]">Volumen diario de turnos agendados en la plataforma.</p>
                </div>
                <span className="rounded-full bg-[#F5F5F7] px-3 py-1 text-xs font-bold text-[#1D1D1F]">
                  {stats.perDay.reduce((a, d) => a + d.count, 0)} turnos en total
                </span>
              </div>
                  ...
                  <div key={d.key} className="group relative flex h-full flex-1 items-end" title={`${d.key}: ${d.count} turnos`}>
                    <div
                      className={`w-full rounded-t-lg transition-all ${
                        i === stats.perDay.length - 1 ? "bg-black" : "bg-[#1D1D1F]/70 hover:bg-black"
                      }`}
                      style={{ height: `${d.count === 0 ? 4 : Math.max(8, (d.count / stats.maxDay) * 100)}%` }}
                    />
                    <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-1 -translate-x-1/2 whitespace-nowrap rounded-lg bg-black px-2 py-1 text-[10px] font-bold text-white opacity-0 group-hover:opacity-100">
                      {d.count} · {d.key.slice(5)}
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-2 flex gap-1">
                {stats.perDay.map((d) => (
                  <span key={d.key} className="flex-1 text-center text-[10px] font-medium text-[#6E6E73]">
                    {d.label}
                  </span>
                ))}
              </div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="rounded-2xl border border-black/[0.08] bg-white p-6 shadow-xs">
                <div className="flex items-center justify-between">
                  <h3 className="font-display text-base font-bold text-[#1D1D1F]">Ranking por turnos</h3>
                  <button onClick={() => setTab("negocios")} className="text-xs font-bold text-[#16A34A] hover:underline">Ver todos →</button>
                </div>
                <p className="text-xs text-[#6E6E73] mt-0.5">Negocios con mayor actividad histórica.</p>
                <ol className="mt-4 space-y-2.5">
                  {stats.topByBookings.length === 0 && <li className="text-sm text-[#6E6E73]">Todavía no hay datos.</li>}
                  {stats.topByBookings.map((t, i) => (
                    <li key={t.user.id} className="flex items-center gap-3 rounded-xl border border-black/[0.06] bg-[#F5F5F7] px-3.5 py-2">
                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-black font-display text-xs font-bold text-white">{i + 1}</span>
                      <span className="min-w-0 flex-1">
                        <span className="block truncate font-display text-xs font-bold text-[#1D1D1F]">{t.user.business}</span>
                        <span className="block text-[10px] text-[#6E6E73]">{t.bookings} turnos · Plan {PLAN_META[t.user.plan].name}</span>
                      </span>
                      <button onClick={() => { loginAs(t.user.id); window.location.hash = "#/app"; }} className="rounded-full bg-black px-3 py-1.5 text-[11px] font-bold text-white hover:bg-neutral-800 transition-colors">Entrar →</button>
                    </li>
                  ))}
                </ol>
              </div>
              ...
              <div className="rounded-2xl border border-black/[0.08] bg-white p-6 shadow-xs">
                <div className="flex items-center justify-between">
                  <h3 className="font-display text-base font-bold text-[#1D1D1F]">Ranking por ingresos estimados</h3>
                  <button onClick={() => setTab("dinero")} className="text-xs font-bold text-[#16A34A] hover:underline">Ver dinero →</button>
                </div>
                <p className="text-xs text-[#6E6E73] mt-0.5">Estimado según precios por turnos confirmados.</p>
                <ol className="mt-4 space-y-2.5">
                  {stats.topByRevenue.length === 0 && <li className="text-sm text-[#6E6E73]">Todavía no hay datos.</li>}
                  {stats.topByRevenue.map((t, i) => (
                    <li key={t.user.id} className="flex items-center gap-3 rounded-xl border border-black/[0.06] bg-[#F5F5F7] px-3.5 py-2">
                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-black font-display text-xs font-bold text-white">{i + 1}</span>
                      <span className="min-w-0 flex-1">
                        <span className="block truncate font-display text-xs font-bold text-[#1D1D1F]">{t.user.business}</span>
                        <span className="block text-[10px] text-[#6E6E73]">{fmtMoney(t.revenue)} estimados</span>
                      </span>
                      <button onClick={() => setEditingUser(t.user)} className="rounded-full border border-black/10 bg-white px-3 py-1.5 text-[11px] font-bold text-[#1D1D1F] hover:bg-[#F5F5F7]">Editar</button>
                    </li>
                  ))}
                </ol>
              </div>
            ...
            {(stats.expired > 0 || stats.expiring7 > 0 || stats.inactive7 > 0) && (
              <div className="rounded-2xl border-2 border-amber-300 bg-amber-50 p-5">
                <p className="flex items-center gap-2 font-display text-base font-extrabold text-amber-950"><AlertCircle className="h-5 w-5 text-amber-600" /> Hoy te conviene atender esto primero</p>
        ...
        {tab === "negocios" && (
          <div className="pop-in mt-6 space-y-6">
            <div className="rounded-2xl border border-emerald-200/70 bg-emerald-50/60 p-4">
              <p className="flex items-center gap-2 font-display text-sm font-extrabold text-emerald-950"><Building2 className="h-4 w-4 text-emerald-700" /> Todos tus locales, con buscador.</p>
                ...
                <div key={u.id} className="card card-hover p-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      >
                        <ArrowRight className="h-3.5 w-3.5" /> Entrar como dueño
                      </button>
                      ...
                      >
                        <span className="inline-flex items-center gap-1">Ver página <ExternalLink size={12} /></span>
                      </a>
        ...
        {tab === "dinero" && (
          <div className="pop-in mt-6 space-y-6">
            <div className="rounded-2xl border border-emerald-200/70 bg-emerald-50/60 p-4">
              <p className="flex items-center gap-2 font-display text-sm font-extrabold text-emerald-950"><DollarSign className="h-4 w-4 text-emerald-700" /> Rendimiento económico</p>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <SaasKpi icon={<DollarSign className="h-5 w-5 text-emerald-700" />} value={fmtMoney(stats.estimatedMonthly)} title="Entra / mes (MRR)" sub={`Por año: ${fmtMoney(stats.arr)}.`} money />
              <SaasKpi icon={<CreditCard className="h-5 w-5 text-inkmute" />} value={fmtMoney(stats.avgTicket)} title="Ticket promedio" sub="Lo que paga en promedio cada que sí paga." />
              <SaasKpi icon={<Users className="h-5 w-5 text-inkmute" />} value={`${stats.crece} × $9.500`} title={`Crece = ${fmtMoney(stats.crece * 9500)}`} sub="Tu plan más popular. El que usa casi todo el mundo." />
              <SaasKpi icon={<TrendingUp className="h-5 w-5 text-inkmute" />} value={`${stats.escala} × $22.000`} title={`Escala = ${fmtMoney(stats.escala * 22000)}`} sub="Tus clientes más grandes. Cuidalos mucho." />
            </div>
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="card p-6">
                <h3 className="flex items-center gap-2 font-display text-lg font-extrabold text-ink"><Clock className="h-5 w-5 text-amber-500" /> Próximos vencimientos (7 días)</h3>
              ...
              <div className="card p-6">
                <h3 className="flex items-center gap-2 font-display text-lg font-extrabold text-ink"><AlertCircle className="h-5 w-5 text-rose-500" /> Vencidos: a recuperar hoy</h3>
                ...
                <div className="mt-4 space-y-2.5">
                  {users.filter((u) => getSubscriptionStatus(u).isExpired).length === 0 && (
                    <p className="rounded-xl bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-900">Cero cuentas vencidas. Todo al día.</p>
                  )}
                ...
                <div className="mt-4 rounded-xl bg-slate-900 p-4 text-white">
                  <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-emerald-300"><Sparkles className="h-3.5 w-3.5 text-emerald-400" /> Cómo cobrar en 1 minuto</p>
            ...
            <div className="card p-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h3 className="flex items-center gap-2 font-display text-lg font-extrabold text-ink"><CreditCard className="h-5 w-5 text-inkmute" /> Registro de suscripciones y pagos</h3>
                ...
                <table className="w-full min-w-[640px] text-left text-xs">
                  ...
                  <tbody>
                    {[...users].sort((a, b) => (a.subscription?.nextRenewal || 0) - (b.subscription?.nextRenewal || 0)).slice(0, 20).map((u) => {
                      ...
                      const auto = (u.subscription as UserSubscription | undefined)?.mpPreapprovalId ? "Automático (MP)" : u.plan === "semilla" ? "— Gratis" : "Manual";
                      ...
                      return (
                        <tr key={u.id} className="border-b border-ink/5 hover:bg-paper">
                          ...
                          <td className="py-2">
                            {u.plan === "semilla" ? (
                              <span className="text-inkmute font-medium">Gratis</span>
                            ) : st.isExpired ? (
                              <span className="inline-flex items-center gap-1 font-semibold text-rose-600"><span className="h-1.5 w-1.5 rounded-full bg-rose-600" /> Vencido</span>
                            ) : st.isGracePeriod ? (
                              <span className="inline-flex items-center gap-1 font-semibold text-amber-600"><span className="h-1.5 w-1.5 rounded-full bg-amber-600" /> En gracia</span>
                            ) : st.isExpiringSoon ? (
                              <span className="inline-flex items-center gap-1 font-semibold text-amber-600"><span className="h-1.5 w-1.5 rounded-full bg-amber-600" /> {st.daysRemaining}d</span>
                            ) : (
                              <span className="inline-flex items-center gap-1 font-semibold text-emerald-600"><span className="h-1.5 w-1.5 rounded-full bg-emerald-600" /> Al día</span>
                            )}
                          </td>
                        </tr>
                      );
        ...
        {tab === "alertas" && (
          <div className="pop-in mt-6 space-y-6">
            <div className="rounded-2xl border border-amber-300 bg-amber-50 p-4">
              <p className="flex items-center gap-2 font-display text-sm font-extrabold text-amber-950"><AlertCircle className="h-4 w-4 text-amber-600" /> Atención prioritaria</p>
              <p className="mt-0.5 text-xs text-amber-900/80">Empezá por los vencidos (urgente), después próximos a vencer y al final locales sin actividad.</p>
            </div>
            <div className="grid gap-6 lg:grid-cols-2">
              <AlertCard
        ...
        {tab === "sistema" && (
          <div className="pop-in mt-6 grid gap-6 lg:grid-cols-2">
            <div className="card p-6">
              <h3 className="flex items-center gap-2 font-display text-lg font-extrabold text-ink"><Shield className="h-5 w-5 text-emerald-600" /> Nube y sincronización</h3>
              <p className="mt-1 text-sm text-inkmute">Estado de persistencia centralizada de datos en Supabase.</p>
              <div className={`mt-4 rounded-2xl border p-4 ${isCloudSyncActive ? "border-emerald-300 bg-emerald-50" : "border-amber-300 bg-amber-50"}`}>
                <p className="flex items-center gap-2 font-display text-sm font-extrabold">
                  <span className={`h-2.5 w-2.5 rounded-full ${isCloudSyncActive ? "bg-emerald-600 animate-pulse" : "bg-amber-500"}`} />
                  {isCloudSyncActive ? "Nube activa y sincronizando" : "Modo local: solo este dispositivo"}
                </p>
                <p className="mt-1 text-xs text-inkmute">{isCloudSyncActive ? "Lo que crees acá aparece al instante en cualquier celular o PC." : "Conectá Supabase para sincronizar automáticamente todos los dispositivos."}</p>
            ...
            <div className="card p-6">
              <h3 className="font-display text-lg font-extrabold text-ink">Acciones rápidas</h3>
              <p className="text-xs text-inkmute">Acceso directo a las tareas frecuentes de administración.</p>
              <div className="mt-4 grid gap-2.5">
                <button onClick={() => setShowNewModal(true)} className="flex items-center justify-between rounded-2xl bg-lime px-4 py-3.5 font-display text-sm font-extrabold text-ink hover:bg-limedeep"><span>＋ Crear negocio nuevo</span> <span>→</span></button>
                <button onClick={() => { setTab("alertas"); }} className="flex items-center justify-between rounded-2xl bg-slate-900 px-4 py-3.5 font-display text-sm font-extrabold text-white hover:bg-slate-800"><span className="flex items-center gap-2"><AlertCircle className="h-4 w-4 text-amber-400" /> Ver qué necesita atención</span> <span>→</span></button>
                <button onClick={() => { setTab("dinero"); }} className="flex items-center justify-between rounded-2xl border-2 border-ink/10 bg-paper px-4 py-3.5 font-display text-sm font-extrabold text-ink hover:border-ink"><span className="flex items-center gap-2"><DollarSign className="h-4 w-4 text-emerald-600" /> Revisar cobros</span> <span>→</span></button>
              </div>
              <p className="mt-4 rounded-xl bg-paper px-4 py-3 text-xs leading-relaxed text-inkmute">Tip: si algo no se actualiza inmediatamente, recargá la página. El estado de la sincronización en la nube se muestra en la barra superior.</p>
            </div>
          </div>
        )}
      </main>
...
function SaasKpi({ icon, emoji, value, title, sub, money = false, small = false, alert = false, accent = false }: { icon?: React.ReactNode; emoji?: string; value: string; title: string; sub: string; money?: boolean; small?: boolean; alert?: boolean; accent?: boolean }) {
  return (
    <div className={`card p-5 shadow-sm ${alert ? "!border-rose-300 !bg-rose-50/50" : accent ? "!border-amber-300 !bg-amber-50/60" : money ? "!border-emerald-300 !bg-emerald-50/50" : ""}`}>
      {icon ? (
        <div className="mb-2 text-ink/70" aria-hidden="true">{icon}</div>
      ) : emoji ? (
        <span className="text-2xl" aria-hidden="true">{emoji}</span>
      ) : null}
      <p className={`font-display font-extrabold text-ink ${small ? "text-2xl" : "text-3xl"} ${money ? "!text-emerald-800" : ""}`}>{value}</p>
...
function AlertCard({ icon, emoji, title, desc, empty, items, actionLabel, onAction, onOpen }: {
  icon?: React.ReactNode; emoji?: string; title: string; desc: string; empty: string;
  ...
  return (
    <div className="card p-6">
      <p className="flex items-center gap-2 font-display text-lg font-extrabold text-ink">
        {icon || (emoji ? <span aria-hidden="true">{emoji}</span> : null)}
        <span>{title}</span>
      </p>
        ...
        <div className="flex items-center justify-between border-b border-ink/10 pb-4">
          <div className="flex items-center gap-3">
            <span className={`flex h-10 w-10 items-center justify-center rounded-xl font-bold text-base ${isConfigured ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>
              {isConfigured ? <Shield className="h-5 w-5 text-emerald-700" /> : <AlertCircle className="h-5 w-5 text-amber-700" />}
            </span>
            <div>
        ...
        <div className="mt-5 space-y-4 text-sm text-ink">
          {isConfigured ? (
            ...
            <div className="rounded-2xl border border-amber-500/40 bg-amber-50 p-4 text-amber-900">
              <p className="flex items-center gap-2 font-bold"><AlertCircle className="h-4 w-4 text-amber-600 shrink-0" /> Actualmente estás en Modo Local (localStorage)</p>
